...
def artifact_edited(job_id: str, payload: EditedArtifactInput, session: Session = Depends(get_db_session)) -> Any:
    settings = get_settings()
    repo = TrackerRepository(session)
    ...
    row = repo.create_writing_delta(delta_id=str(uuid4()), job_id=job_id, artifact_name=payload.artifact_name, original_text=original_text, final_text=payload.final_text, delta_json=delta, user_id=settings.user_id)
    record_signal(repo, Signal(signal_type="artifact_edit", job_id=job_id, payload={"artifact_name": payload.artifact_name, "delta": delta}, user_id=settings.user_id, run_id="api"))
    return row
