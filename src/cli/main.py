...
from src.services.application_service import execute_application
from src.services.linkedin_diagnostic_service import run_linkedin_diagnostic, run_linkedin_session_setup
...
def _record_job_signal(job_id: str, signal_type: str, payload: dict[str, object]) -> None:
    ...
    with get_session() as session:
        ...
        record_signal(repo, Signal(signal_type=signal_type, job_id=job_id, payload=payload, user_id=settings.user_id))
...
def _set_application_status(job_id: str, status: str, notes: str = "") -> None:
    ...
    with get_session() as session:
        repo = TrackerRepository(session)
        updated = repo.update_application_status(job_id, status, notes=notes, user_id=settings.user_id)
        if updated is None:
            repo.upsert_application(ApplicationRecord(id=f"app-{job_id}", job_id=job_id, status=status), link="", notes=notes, user_id=settings.user_id)
...
def run(
    ...
    rss_feed: str = typer.Option("https://remoteok.com/remote-dev-jobs.rss", help="RSS URL"),
    manual_url: list[str] = typer.Option([], help="Manual URL(s) for job descriptions"),
) -> None:
    init_db()
    settings = get_settings()
    profile = load_profile(settings.profile_path)
    run_id = run_pipeline(profile=profile, sources=[item.strip() for item in sources.split(",")], limit=limit, rss_urls=[rss_feed], manual_urls=manual_url)
    typer.echo(f"Run completed: {run_id}")


@app.command(name="list")
def list_jobs(top: int = typer.Option(20), min_score: int = typer.Option(0), explore: bool = typer.Option(False, help="Enable 80/20 exploration mode")) -> None:
    settings = get_settings()
    with get_session() as session:
        rows = TrackerRepository(session).list_recommendations(limit=top, min_score=min_score, user_id=settings.user_id, explore=explore)
    for item in rows:
        row = item["job"]
        breakdown = breakdown_for_job(row)
        extra = " [exploration]" if item["is_exploration"] else ""
        typer.echo(f"{row.id} | score: {row.score} | {row.company} | {row.title}{extra}\n[skills +{breakdown.get('skill_match_score', 0)} | seniority +{breakdown.get('seniority_score', 0)} | location +{breakdown.get('location_score', 0)} | keywords +{breakdown.get('keyword_density_score', 0)} | red_flags -{breakdown.get('red_flag_penalty', 0)}]")
...
def apply(job_id: str) -> None:
    settings = get_settings()
    with get_session() as session:
        result = execute_application(TrackerRepository(session), settings, job_id, settings.user_id, trigger="cli")
    typer.echo(str(result))


@app.command()
def diagnose_linkedin(job_url: str) -> None:
    init_db()
    settings = get_settings()
    with get_session() as session:
        result = run_linkedin_diagnostic(TrackerRepository(session), settings, job_url)
    typer.echo(str(result))


@app.command()
def setup_linkedin_session() -> None:
    init_db()
    settings = get_settings()
    with get_session() as session:
        result = run_linkedin_session_setup(TrackerRepository(session), settings)
    typer.echo(str(result))
...
def tools() -> None:
    for tool in list_tools():
        typer.echo(f"{tool['name']} [{tool['category']}] - {tool['description']}")


@app.command()
def approve(approval_id: str, yes: bool = typer.Option(False), no: bool = typer.Option(False), reason: str = typer.Option("like_role"), notes: str = typer.Option("")) -> None:
    settings = get_settings()
    if yes == no:
    ...
    status_value = "approved" if yes else "rejected"
    signal_type = "approval" if yes else "rejection"
    with get_session() as session:
        repo = TrackerRepository(session)
        updated = repo.update_approval_status(approval_id, status_value)
        if not updated:
            typer.echo("Approval not found")
            raise typer.Exit(code=1)
        record_signal(repo, Signal(signal_type=signal_type, job_id=updated.job_id, payload={"approval_id": approval_id, "reason": reason, "notes": notes}, run_id=updated.run_id, user_id=settings.user_id))
    typer.echo(f"Approval {approval_id} -> {status_value}")


@app.command()
def reject(job_id: str, reason: str = typer.Option("stack_mismatch"), notes: str = typer.Option("")) -> None:
...
@app.command()
def interview(job_id: str, date_value: str = typer.Option(..., "--date"), notes: str = typer.Option("")) -> None:
...
@app.command()
def artifact_edit(job_id: str, name: str = typer.Option(...), file: Path = typer.Option(...)) -> None:
    settings = get_settings()
    ...
    with get_session() as session:
        ...
        repo.create_writing_delta(delta_id=str(uuid4()), job_id=job_id, artifact_name=name, original_text=original_text, final_text=final_text, delta_json=delta, user_id=settings.user_id)
        record_signal(repo, Signal(signal_type="artifact_edit", job_id=job_id, payload={"artifact_name": name, "delta": delta}, user_id=settings.user_id))
...
def feed_list(hiring_only: bool = typer.Option(False, "--hiring-only")) -> None:
    settings = get_settings()
    with get_session() as session:
        rows = TrackerRepository(session).list_feed_items(hiring_only=hiring_only, user_id=settings.user_id)
    for row in rows:
        typer.echo(f"{row.id} | hiring={row.is_hiring} | conf={row.confidence:.2f} | {row.url}")


@app.command()
def export(format: str = typer.Option("csv", help="Export format, currently only csv"), min_score: int = typer.Option(70)) -> None:
    if format != "csv":
...
def followups() -> None:
    ...
    if not rows:
        ...
        return
    for row in rows:
        typer.echo(f"{row.id} | {row.job_id} | {row.status} | due: {row.follow_up_date} | {row.link}")
...
def preferences_show() -> None:
    settings = get_settings()
    with get_session() as session:
        model = load_preference_model(TrackerRepository(session), user_id=settings.user_id)
...
def preferences_reset() -> None:
    settings = get_settings()
    with get_session() as session:
        TrackerRepository(session).upsert_preference_model("default", DEFAULT_WEIGHTS, user_id=settings.user_id)
